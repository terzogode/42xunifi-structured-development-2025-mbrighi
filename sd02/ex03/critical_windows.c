...
/*   Updated: 2025/06/19 16:36:45 by mbrighi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "critical_windows.h"

static int	is_valid_window(const int *readings, int start)
{
	int	avg = 0;
	int	crit_read_tmp = 0;
	int	index_tmp = 0;
...
int	count_critical_windows(const int *readings, int size)
{
	...
	while (idx <= size - 5)
	{
		if (is_valid_window(readings, idx))
			ret++;
		idx++;
	}
	return (ret);
